<template>
  ...
  <form @submit.prevent="submitForm" class="flex flex-col gap-2 max-w-md my-4">
      <div> 
          ...
          <input required v-model="formName" type="text" id="text" aria-describedby="helper-text-explanation" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-cyan-500 focus:border-cyan-500 block w-full p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-cyan-500 dark:focus:border-cyan-500" placeholder="Ваше имя">
      </div>
      <div> 
          <label for="email" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Ваш email</label>
          <input required v-model="formEmail" type="email" id="email" aria-describedby="helper-text-explanation" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-cyan-500 focus:border-cyan-500 block w-full p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-cyan-500 dark:focus:border-cyan-500" placeholder="name@academy.top">
      </div>
      <div>
          <label for="message" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Ваше сообщение</label>
          <textarea required v-model="formMessage" id="message" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-cyan-500 focus:border-cyan-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-cyan-500 dark:focus:border-cyan-500" placeholder="Ваше сообщение..."></textarea>
      </div>
      <button type="submit" class="text-white bg-cyan-600/70 hover:bg-cyan-600/80 focus:ring-4 focus:outline-none focus:ring-blum-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-cyan-600/60 dark:hover:bg-cyan-600/70 dark:focus:ring-cyan-600/80">Отправить</button>
  </form>
</template>
